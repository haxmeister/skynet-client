...
FAMYTools.Multispot = function(table)
local multi_list = {}
	for _,player in ipairs(table or {}) do
		local guild = "" 
...
FAMYTools.Singlespot = function(table)
	for idx, player in ipairs(table) do
		if (player.name~="") then
			local guild = "" 
			if ((player.guildtag or "")~="") then
				guild = "[" .. player.guildtag .. "] "
...
FAMYTools.Refuel = function(table)
	local exists = false
	local txt = ""
	local guild = "" 
	if ((table.guildtag or "")~="") then
		guild = "[" .. table.guildtag .. "] "
	end
	local reporterguild = "" 
	if ((table.reporterguild or "")~="") then
		reporterguild = "[" .. table.reporterguild .. "] "
	end
	local factioncolor = ""
	if ((tonumber(table.faction) or 0) > 0) then
		factioncolor = rgbtohex(FactionColor_RGB[tonumber(table.faction)])
	else
		factioncolor = '\127FF0000'
	end
	local reporterfactioncolor = ""
	if ((tonumber(table.reporterfaction) or 0) > 0) then
		reporterfactioncolor = rgbtohex(FactionColor_RGB[tonumber(table.reporterfaction)])
	else
		reporterfactioncolor = '\127FF0000'
	end
	local location = "Unknown Location"
	local sectid = table.sectorid or -1
	if (sectid>0) then
		location = ShortLocationStr(sectid)
	end
	local shipname = (table.shipname) or ""
	table.t = tonumber(table.t or 0)
	txt = "\127FFFFFF" .. guild .. table.name .. '\127cccccc' .. " just refuelled at " .. '\127FFFFFF' .. location .. "\127cccccc" .. shipname .. "." .. '\127FFFFFF'
	return txt
end
...
FAMYTools.Events["playerseen"] = function(params)
	...
	local txt = ""
	txt = 
	FAMYTools.print("lulz2")
for idx, player in ipairs(table) do
  FAMYTools.Singlespot(playerlist)
end
